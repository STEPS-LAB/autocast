...
export async function POST(request: Request) {
  try {
    const rl = rateLimit(request, { bucket: 'admin:upload-category-image', limit: 20, windowMs: 60_000 })
    ...
    const categoryId = parsedBody.data.categoryId.trim()
    ...
    if (!hasBucket) {
      const { error: createBucketError } = await serviceClient.storage.createBucket(bucketName, { public: true })
    ...
    const path = `${categoryId}/${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${extension}`
