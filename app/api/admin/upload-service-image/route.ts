...
export async function POST(request: Request) {
  try {
    const rl = rateLimit(request, { bucket: 'admin:upload-service-image', limit: 20, windowMs: 60_000 })
    ...
    const serviceId = parsedBody.data.serviceId.trim()
    ...
    const path = `${serviceId}/${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${extension}`
